...
public class LinkTargetReorientCommand extends EditElementCommand {
	...
	protected boolean canReorientSource() {
		...
		Target target = (Target) getLink().getTarget().get(0);
		...
		return DsmlBaseItemSemanticEditPolicy.getLinkConstraints()
				.canExistLinkTarget_4004(container, getLink(), getNewSource(),
						target);
	}
	...
	protected boolean canReorientTarget() {
		...
		Component source = getLink().getSource();
		if (!(getLink().eContainer() instanceof DrawingArea)) {
			return false;
		}
		DrawingArea container = (DrawingArea) getLink().eContainer();
		return DsmlBaseItemSemanticEditPolicy.getLinkConstraints()
				.canExistLinkTarget_4004(container, getLink(), source,
						getNewTarget());
	}
